...
def ds_generator(data, shape):
    ...
    # TODO: find if we are training the model on BGR images instead of RGB
    for rgb_path, d_path in data:
        [rgb, d] = load_data([(rgb_path, d_path)])
        ...
        rgb, d = preprocess_data((rgb, d))
        yield rgb, d


def re_align(rgb_img, delta):
    # TODO: Implement this function as a way to realign rgb images with d images
    rgb_img = rgb_img[:delta[0], :delta[1], :]
    return rgb_img


def normalize_data(data):
    # Normalize rgb and depth data
    # TODO: find out if we really want to preprocess data in the dataset
    rgb_img = data[0].astype('float32')
    d_img = data[1].astype('float32')
